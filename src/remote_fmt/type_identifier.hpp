...
namespace remote_fmt { namespace detail {
    ...
    template<typename T, typename Append>
    void appendSized(TimeSize ts, T v, Append append) {
        switch(ts) {
    ...
    template<typename T, typename Append>
    void appendSized(RangeSize rs, T v, Append append) {
        switch(rs) {
    ...
    template<typename T>
    constexpr std::byte castAndShift(T v, std::size_t shift) {
        return static_cast<std::byte>(
          static_cast<std::uint8_t>(v) << static_cast<std::uint8_t>(shift));
    }
    ...
    constexpr std::optional<RangeSize>
    parseFmtStringTypeIdentifier(std::byte v, FmtStringType type) {
        ...
        if(
          v
          != (detail::castAndShift(ti, 0) | detail::castAndShift(ft, 4) | detail::castAndShift(ts, 2)))
        {
            return std::nullopt;
        }
    ...
    constexpr std::optional<std::pair<TrivialType, TypeSize>>
    parseTrivialTypeIdentifier(std::byte v) {
        ...
        if(
          v
          != (detail::castAndShift(ti, 0) | detail::castAndShift(tt, 4) | detail::castAndShift(ts, 2)))
        {
            return std::nullopt;
        }
        return {
    ...
    constexpr std::optional<std::tuple<RangeType, RangeSize, RangeLayout>>
    parseRangeTypeIdentifier(std::byte v) {
        ...
        if(
          v
          != (detail::castAndShift(ti, 0) | detail::castAndShift(rs, 2) | detail::castAndShift(rt, 4) | detail::castAndShift(rl, 7)))
        {
            return std::nullopt;
        }
        return {
    ...
    constexpr std::optional<std::tuple<TimeType, TypeSize, TypeSize, TimeSize>>
    parseTimeTypeIdentifier(std::byte v) {
        ...
        if(
          v
          != (detail::castAndShift(ti, 0) | detail::castAndShift(num_ts, 2) | detail::castAndShift(den_ts, 4) | detail::castAndShift(ts, 6) | detail::castAndShift(tt, 7)))
        {
            return std::nullopt;
        }
        return {
