...
namespace detail {
    ...
    static inline std::optional<std::string> formatTime(
      std::uint64_t    num,
      std::uint64_t    den,
      std::int64_t     value,
      TimeType         tt,
    ...
    ParseResult<It> parseString(
      ...
      bool             in_list) {
        ...
        std::transform(first, string_end, s.begin(), [](auto b) { return static_cast<char>(b); });
        ...
        static ParseResult<It> parse(
          ...
          std::map<std::uint32_t, std::string> const& stringConstantsMap) {
            ...
            auto extractColor = [&](bool rgb, bool term, auto gen) {
                if(rgb) {
                    auto const o = extractSize(first, last, TypeSize::_4);
                    if(!o) {
                        return false;
                    }
                    first = o->second;
                ...
                if(term) {
                    auto const o = extractSize(first, last, TypeSize::_1);
                    if(!o) {
                        return false;
                    }
                    first = o->second;
    ...
    static inline std::optional<RangeReplacementField>
        ...
        while(size != 0 && first != last) {
            auto const os = [&]() {
                ...
                return parseFromTypeId(
                  ...
                  childReplacementField,
                  true,
                  rt == RangeType::map || rangeReplacementField.find('m') != std::string_view::npos,
                  stringConstantsMap);
            }();
            if(!os) {
                return std::nullopt;
    ...
    ParseResult<It> parseType(
      ...
      std::map<std::uint32_t, std::string> const& stringConstantsMap) {
        ...
        case TypeIdentifier::range:
            return parseRange(first, last, replacementField, in_list, in_map, stringConstantsMap);
        }
        return std::nullopt;
    }
      ...
      std::map<std::uint32_t, std::string> const& stringConstantsMap) {
        ...
        if(ti == TypeIdentifier::fmt_string) {
            if(parseFmtStringTypeIdentifier(*first, FmtStringType::sub)) {
                auto const oFmtTypeSize = parseFmtStringTypeIdentifier(*first, FmtStringType::sub);
                if(!oFmtTypeSize) {
                    return std::nullopt;
                }
                if(replacementField != "{}") {
